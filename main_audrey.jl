...
capex_offshore = capex_offshore_fixed #€/MW
capex_solar = capex_solar_ground #€/MW
opex_offshore = opex_offshore_fixed #€/MW/year
opex_solar = opex_solar_ground #€/MW/year
...
@objective(model, Min, 
                        ...
                        + Pmax_h2*sum(H2installed[g] for g in 1:NH2_max)*(capex_CCG_H2 + opex_CCG_H2) + sum(PH2[t,g] for t in 1:Tmax, g in 1:NH2_max)*cH2
                        + sum(Puns[t]*cuns[t] for t in 1:Tmax) + sum(Pexc[t]*cexc[t] for t in 1:Tmax))
