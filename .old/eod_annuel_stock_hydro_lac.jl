...
hydro_stock_remaining = zeros(LAST_WEEK)
hydro_stock_available = zeros(LAST_WEEK)
hydro_utilization_rate = zeros(LAST_WEEK)
...
for week in 1:LAST_WEEK
    ...
    e_hy_total = e_hy_lacs + stock_hydro_lac_initial
    ...
    @constraint(model, sum(Phy[t] for t in 1:Tmax) <= e_hy_total)
    ...
    remaining_hydro = e_hy_total - production_hydro_week
    

    hydro_stock_available[week] = e_hy_total
    hydro_stock_remaining[week] = remaining_hydro
    hydro_utilization_rate[week] = production_hydro_week / e_hy_total
...
open("results/annual_stock_hydro_lac/results.csv", "w") do f
    write(f, "t;Solar;Onshore;Offshore;Battery_stock;Battery_charge;Battery_discharge;STEP_stock;STEP_charge;STEP_discharge;H2_CCG;H2_TAC;Hydro;Hydro_lac_utilization_rate;hy_th_fatal;Load;Defailance;Exces\n")
    for t in 1:Nhours
        write(f,
            string(
                t, ";",
                ...
                round(hydro_utilization_annual[t], digits=4), ";",
...
open("results/annual_stock_hydro_lac/parc_annuel.json", "w") do f
...
# Écriture dans fichier JSON
open("results/annual_stock_hydro_lac/evolution_parc.json", "w") do f
