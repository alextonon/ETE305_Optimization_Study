...
e_STEP = XLSX.readdata(data_file, "data", "S5")
...
#############################
#balance constrain
@constraint(model, balance[t in 1:Tmax], sum(Pth[t,g] for g in 1:Nth) + sum(Phy[t,h] for h in 1:Nhy) + Pres[t] + Pdecharge_STEP[t] - Pcharge_STEP[t] + Puns[t] - load[t] - Pexc[t] == 0)
...
@constraint(model, stock_hy, e_hy .- sum(Phy[1:Tmax,1:Nhy])  >= 0)
#weekly STEP
@constraint(model, STEP_charge_max[t in 1:Tmax], 0 <= Pcharge_STEP[t]<=Pmax_STEP)
@constraint(model, STEP_decharge_max[t in 1:Tmax], 0 <= Pdecharge_STEP[t]<=Pmax_STEP)
@constraint(model, STEP_charge_x_0, Pcharge_STEP[1]==0)
@constraint(model, STEP_decharge_0, Pdecharge_STEP[1]==0)

stock_initial_STEP = 0.0 
@constraint(model, STEP_stock_balance_1, 
    stock_STEP[1] == stock_initial_STEP + rSTEP * Pcharge_STEP[1] - Pdecharge_STEP[1])
@constraint(model, STEP_stock_balance[t in 2:Tmax], 
    stock_STEP[t] == stock_STEP[t-1] + rSTEP * Pcharge_STEP[t] - Pdecharge_STEP[t])
